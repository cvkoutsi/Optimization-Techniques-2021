...
%find min of function using ã found by using the Golden Section Method
...
fprintf('Lavenberg Marquardt Method- Find gk using the golden section method\n');
for i = 1:3
    x = x_start(i);
    y = y_start(i);
    ...
    while norm(gradf(x,y)) > epsilon
        ...
        l_limit = 0;
        u_limit = 5;
        [gamma, ~, ~, ~, ~] = golden(h, l, l_limit, u_limit);
